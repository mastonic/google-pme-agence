...
const PLAN_STYLE = {
    starter: { icon: Zap, color: 'text-blue-400', borderColor: 'border-blue-500/30', bgColor: 'bg-blue-500/10' },
    pro:     { icon: Star, color: 'text-brand', borderColor: 'border-brand/40', bgColor: 'bg-brand/10' },
    elite:   { icon: Crown, color: 'text-amber-400', borderColor: 'border-amber-500/30', bgColor: 'bg-amber-500/10' },
};

const featureText = (feature) => typeof feature === 'string' ? feature : feature?.text;
const includedFeatures = (features = []) => features
            ...
            <div className="relative z-10 w-full max-w-3xl bg-slate-900 border border-white/10 rounded-2xl shadow-2xl overflow-hidden">
                ...
                <div className="p-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {plans.map((plan) => {
                        ...
                        const features = includedFeatures(plan.features);
                        const isLoading = loadingPlan === plan.slug;
                        return (
                            <div
                                key={plan.slug}
                                className={`relative flex flex-col rounded-2xl border p-5 transition-all ${style.borderColor} ${style.bgColor} ${
                                    plan.is_popular ? 'ring-2 ring-brand/50' : ''
                                }`}
                            >
                                {plan.is_popular && (
                                ...
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 ${style.bgColor} border ${style.borderColor}`}>
                                    <Icon className={`w-5 h-5 ${style.color}`} />
                                </div>

                                <h3 className={`text-lg font-bold ${style.color} mb-1`}>{plan.name}</h3>
                                <div className="mb-4">
