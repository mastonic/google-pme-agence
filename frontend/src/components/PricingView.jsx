                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {plans.map((plan) => {
                        ...
                        return (
                            <div
                                key={plan.slug}
                                className={`relative flex flex-col rounded-2xl border p-6 transition-all ${style.borderColor} ${style.bgColor} ${
                                    plan.is_popular ? 'ring-2 ring-brand/50 scale-[1.02]' : ''
                                }`}
                            >
                                {plan.is_popular && (
                                ...
                                <h3 className={`text-xl font-bold ${style.color} mb-1`}>{plan.name}</h3>
                                ...
                                <ul className="flex-1 space-y-3 mb-6">
                                    {features.map((f) => (
                                        <li key={f} className="flex items-start gap-2.5 text-sm text-slate-300">
                                            <span className={`mt-0.5 flex-shrink-0 w-5 h-5 rounded-full ${style.badgeBg} flex items-center justify-center`}>
                                ...
                                <div className={`py-2.5 text-center rounded-xl font-bold text-sm ${style.badgeBg} ${style.color} border ${style.borderColor}`}>
                    ...
                    <div className="mt-10 glass p-6 rounded-2xl border border-white/10">
                        ...
                        <div className="grid grid-cols-4 gap-4">
                            {Object.entries(kpis.tier_breakdown).map(([tier, count]) => {
                                ...
                                const style = PLAN_STYLE[tier] || PLAN_STYLE.starter;
                                return (
                                    <div key={tier} className="text-center">
                                        <p className={`text-2xl font-extrabold ${style.color || 'text-slate-400'}`}>{count}</p>
