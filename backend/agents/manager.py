...
RÈGLES : VOUVOIEMENT PARTOUT. Jamais "Je me permets". Ton direct et chaleureux. 14-16 lignes MAX. Tout en français. Sans objet ni balise HTML. N'ÉCRIS JAMAIS de numéros de ligne (L1:, L2:, Ligne 3:), de JSON, de crochets [ ], d'accolades { }, de tableau ni de Markdown. Retourne uniquement le texte naturel de l'email."""
...
VOUVOIEMENT OBLIGATOIRE. 14-16 lignes. Sans objet ni balise HTML. Aucun numéro de ligne, aucun JSON, aucun crochet [ ], aucune accolade { }, aucun tableau ni Markdown."""
    ...
    def _sanitize_sales_email(self, email_text: str) -> str:
        """Remove technical formatting artefacts before an email is shown/sent."""
...
VOUVOIEMENT OBLIGATOIRE. 14-16 lignes. Sans objet ni balise HTML. Aucun numéro de ligne, aucun JSON, aucun crochet [ ], aucune accolade { }, aucun tableau ni Markdown."""
        ...
        try:
            ...
            self._push_log("Le Closer", f"⚠️ Email simplifié : {e}", "chat")
        email_text = self._sanitize_sales_email(email_text)
        if self._sales_email_has_artifacts(email_text):
            self._push_log("Le Closer", "⚠️ Artefacts techniques détectés — régénération propre.", "system")
            clean_prompt = f"""Réécris cet email de prospection en français naturel, sans changer les faits ni l'offre.

EMAIL À NETTOYER :
{email_text[:5000]}

CONTRAINTES ABSOLUES :
- aucun numéro de ligne du type L1:, L2:, Ligne 3:
- aucun JSON
- aucun crochet ni accolade
- aucun tableau ni Markdown
- aucune explication autour de l'email
- conserve la signature Ludovic / Pulse-PME
- conserve uniquement les faits déjà présents

Retourne uniquement le texte final de l'email."""
            email_text = self._sanitize_sales_email(
                self._call(clean_prompt, max_tokens=1600, temperature=0.2)
            )
        email_text = self._finalize_sales_email(email_text)
        return email_text
...
, '', text)
    ...
    def _sales_email_has_artifacts(self, email_text: str) -> bool:
        ...
        patterns = (
            ...
            r'(?m)^\s*[\[\]{}]\s*,?\s*        """Add the real preview URL and direct checkout links to every final sales email."""
        biz = self.business_data
        preview_url = (biz.get("deployment_url") or "").strip()
        payment_links = biz.get("payment_links") or {}
