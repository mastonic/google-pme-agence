...
def _parse_first_json_value(raw: str):
    """Parse the first valid JSON value from noisy LLM output.

    Accepts markdown fences, prose around JSON and concatenated JSON values.
    json.JSONDecoder.raw_decode deliberately ignores trailing content, avoiding
    the common "Extra data" failure from model responses.
    """
    text = (raw or "").strip()
...
## PREUVES SOCIALES
Avis clients : utilise uniquement les avis réels fournis dans les données. S’il n’y en a pas, indique qu’aucun avis vérifié n’est disponible et n’invente rien.
...
COMMENCE DIRECTEMENT par <!DOCTYPE html>"""
    ...
    # Legacy Jinja templates were removed from the repository.
    ...
    def _extract_content_slots(self, prep_data: dict) -> dict:
        ...
        if positive:
            ...
            reviews_block = "Aucun avis client vérifié disponible — n'invente aucun témoignage. Laisse testimonials vide."
...
{{
  ...
  "about": {{
    ...
    "highlight": "1 fait fort uniquement s’il est sourcé; sinon une formulation qualitative sans chiffre"
  }},
  "offerings": [
    {{
...
- testimonials : reprends uniquement les avis positifs réels fournis. S’il n’y en a pas, retourne [] et n’invente rien
- stats[2] : utilise seulement une ancienneté explicitement sourcée. Sinon mets "Qualité · Proximité"
- Tout en FRANÇAIS sauf les noms propres"""
        ...
        try:
            ...
            parsed = _parse_first_json_value(raw)
            if not isinstance(parsed, dict):
                raise ValueError("Le contenu structuré doit être un objet JSON")
            slots = parsed
    ...
    def _render_from_template(self, content_slots: dict, all_photos: list) -> str:
        ...
        if not _os.path.exists(tpl_path):
            self._push_log("L'Ingénieur", f"ℹ️ Template legacy {template_name}.html absent — moteur Design V2 utilisé", "system")
            return ""
...
(function(){
    ...
    def run_build_crew(self, prep_data: dict) -> dict:
        ...
        # The removed Jinja templates caused guaranteed fallback warnings.
...
, '', text)
    ...
    for i, ch in enumerate(text):
        if ch not in "{[":
            continue
        candidate = _sanitize_json_strings(text[i:])
